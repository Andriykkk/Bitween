...
model_configs = {
    ...
    "Medium (n_embd=768, n_layer=12)": GPTConfig(
        vocab_size=50257, block_size=max(context_sizes), n_embd=768, n_head=12, n_layer=6, dropout=0.1
    ),
    "Large (n_embd=1024, n_layer=18)": GPTConfig(
        vocab_size=50257, block_size=max(context_sizes), n_embd=1024, n_head=16, n_layer=6, dropout=0.1
    ),
}
...
# Plot Inference Time
ax1.set_title('Inference Time vs. Context Size')
...
ax2.set_title('Training Step (Forward + Backward) vs. Context Size')
...
print("Saved performance analysis plots to performance_analysis.png")